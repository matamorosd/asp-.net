...
Partial Class Control_Inventario_Entrada_Inventario
    ...
    'Public Shared Function BuscarMateriales(pProveedor As String) As String
    ...
    Public Shared Function BorrarMaterial(pReferencia As String, pUsuarioNom As String) As String
        ...
        Dim cnn As New conexion
        Dim Sql As String = "delete from entrada_detalle_temporal where entrada_codigo_temporal = '" + pUsuarioNom + "' and entrada_material_temporal = '" + pReferencia + "'"
        Try
    ...
    <WebMethod(EnableSession:=True)>
    Public Shared Function Grabando(pFactura As String, pProveedor As String, pComentario As String, pUsuario As String, pUsuarioNom As String) As String
        Dim cUser As New Usuario
        Dim Retorno As String = "0"
        Dim cnn As New conexion
        Dim SqlCorrelativo As String = "select max(entrada_codigo)+1 from entrada_inventario"
        ...
        If IsNothing(Correlativo) Or Correlativo = 0 Then
            Correlativo = 1
        End If
        Dim Sql As String = "insert into entrada_inventario set entrada_codigo = '" + Correlativo.ToString + "', entrada_factura = '" + pFactura + "', entrada_fecha = now(), entrada_proveedor = '" + pProveedor + "', entrada_usuario = '" + pUsuario + "', entrada_comentario = '" + pComentario + "'"
        Try
            Retorno = cnn.EjecutarInsertUpdateInventario(Sql)
        Catch ex As Exception
            'Retorno = ex.Message
        End Try
        'GRABANDO EL DETALLE DE LOS DETALLES DEL CONTROL
        If Retorno = 1 Then
            'Dim SqlCuenta As String = "select count(*) from  where control_codigo_temp = '" + pUsuarioNom + "'"
            'If cnn.getValorQuery(SqlCuenta) = 0 Then
            'Else
            Dim SqlDetalle As String = "INSERT INTO entrada_detalle (entrada_Codigo, entrada_material, entrada_cantidad, entrada_subtotal, entrada_moneda) SELECT " + Correlativo.ToString + ", entrada_material_temporal, entrada_cantidad_temporal, entrada_subtotal_temporal, entrada_moneda_temporal FROM entrada_detalle_temporal where entrada_codigo_temporal = '" + pUsuarioNom.ToString + "'"
            Try
        ...
        End If
        If Retorno >= 1 Then
            Dim SqlExistencia As String = "select * from entrada_detalle where entrada_codigo = '" + Correlativo.ToString + "'"
            cnn.cnnInventario.Open()
            cnn.cmdInventario = New MySqlCommand(SqlExistencia, cnn.cnnInventario)
            Dim ReaderInventario As MySqlDataReader = cnn.cmdInventario.ExecuteReader
            While ReaderInventario.Read
                If Not IsDBNull(ReaderInventario("entrada_cantidad")) Then
                    Dim Material As String = ReaderInventario("entrada_material")
                    Dim Existencia As String = ReaderInventario("entrada_cantidad")
